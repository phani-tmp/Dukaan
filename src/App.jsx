...
import React, { useState, useEffect, useCallback, useMemo } from 'react';
// Fix: Removed .js extension from local import path for compatibility
import { firebaseConfig, localAppId } from './firebaseConfig';
...
// Icon Imports 
import { Search, MapPin, ShoppingCart, User, Home, Package, ChevronLeft, Minus, Plus, IndianRupee, Mic, Store, LogOut, CheckCircle, Clock, ShoppingBag, Truck, Check, X, Pencil } from 'lucide-react';
...
// --- AUTHENTICATION VIEW (New Email/Password Login) ---

const AuthView = ({ setAuthReady, setUserId }) => {
  ...
  // Function to convert 10-digit mobile number to a Firebase-compatible email
  const getAuthEmail = (mobileNumber) => {
    // Ensure the number is exactly 10 digits
    const cleanMobile = mobileNumber.replace(/\D/g, '').slice(0, 10);
    if (cleanMobile.length !== 10) {
      return null;
    }
    // Use a placeholder domain to satisfy Firebase Email/Password Auth
    return `${cleanMobile}@dukaan.in`;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const authEmail = getAuthEmail(mobile);
    if (!authEmail) {
    ...
    try {
      if (isSignUp) {
        ...
        setMessage('Signed in successfully! Loading app...');
      }
      // onAuthStateChanged listener handles the rest
    } catch (error) {
      console.error("Auth Error:", error);
      setMessage(`Failed: ${error.message.replace('Firebase: ', '')}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="card p-6" style={{ maxWidth: '400px', margin: '40px auto', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)', padding: '32px' }}>
      <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--color-green-700)', marginBottom: '24px', textAlign: 'center' }}>
        {isSignUp ? 'Shopkeeper Sign Up' : 'Shopkeeper Login'}
      </h2>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <button type="submit" disabled={loading} className="btn-primary" style={{ backgroundColor: '#10b981', fontSize: '1.125rem' }}>
          {loading ? (isSignUp ? 'Signing Up...' : 'Signing In...') : (isSignUp ? 'Create Account' : 'Login')}
        </button>
      </form>

      {message && <p style={{ marginTop: '16px', textAlign: 'center', fontSize: '0.875rem', color: message.includes('success') ? 'var(--color-green-600)' : 'var(--color-red-600)' }}>{message}</p>}

      <button onClick={() => setIsSignUp(!isSignUp)} className="btn-link" style={{ marginTop: '20px', color: '#2563eb' }}>
        {isSignUp ? 'Already have an account? Login' : "Need an account? Sign Up"}
      </button>
      <p style={{ marginTop: '10px', textAlign: 'center', fontSize: '0.75rem', color: '#9ca3af' }}>
        This is for both shopkeepers and buyers.
      </p>
    </div>
  );
};
...
const ShopkeeperDashboard = ({ userId, storeId, setStoreId, setLinkedStoreId, stores, shopOrders, newOrderCount, products }) => {
  const isRegistered = !!storeId;
  const store = stores.find(s => s.id === storeId);
  ...
  // --- Product Price Update Logic (NEW) ---
  const handleUpdatePrice = useCallback(async (productId, newPrice) => {
    const price = parseFloat(newPrice);
    if (isNaN(price) || price <= 0) return;

    try {
      const productRef = doc(db, 'artifacts', appId, 'public', 'data', 'products', productId);
      await updateDoc(productRef, { price: price, updatedAt: new Date().toISOString() });
  ...
  // --- Product Adding Form with Image Handling and Listing ---
  const ProductAddingForm = () => {
    const [pName, setPName] = useState('');
    const [pPrice, setPPrice] = useState(0);
    const [pCat, setPCat] = useState('Groceries');
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        {/* 2. Price Update Section */}
        <div className="card p-4" style={{ border: '1px solid var(--color-gray-200)', padding: '16px' }}>
          <h3 style={{ fontSize: '1.25rem', fontWeight: 700, color: 'var(--color-gray-800)', marginBottom: '16px' }}>Quick Price Update</h3>

          {storeProducts.length === 0 ? (
            <p style={{ color: '#6b7280', textAlign: 'center' }}>Add a product above to see the list here.</p>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {storeProducts.map(product => (
                <div key={product.id} className="flex-between" style={{ borderBottom: '1px solid var(--color-gray-100)', paddingBottom: '8px', alignItems: 'center' }}>
                  <span style={{ fontWeight: 500, fontSize: '0.9rem', width: '50%', color: 'var(--color-gray-800)' }}>{product.name.split('/')[0]}</span>
                  ...
                  {editPriceId === product.id ? (
                    <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
                      <input
                        ...
                        className="input-field"
                        style={{ width: '70px', padding: '6px', fontSize: '0.9rem', textAlign: 'center' }}
                      />
                      <button
                        onClick={() => {
                          handleUpdatePrice(product.id, editPriceValue);
                          setEditPriceId(null);
                        }}
                        style={{ backgroundColor: 'var(--color-green-700)', color: 'white', padding: '6px', borderRadius: '4px' }}
                      >
                        <Check style={{ width: '18px', height: '18px' }} />
                      </button>
                      <button onClick={() => setEditPriceId(null)} style={{ backgroundColor: 'var(--color-red-600)', color: 'white', padding: '6px', borderRadius: '4px' }}>
                        <X style={{ width: '18px', height: '18px' }} />
                      </button>
                    </div>
                  ) : (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <span style={{ fontWeight: 700, display: 'flex', alignItems: 'center', color: 'var(--color-green-700)' }}>
                      ...
                      <button
                        ...
                        style={{ border: '1px solid #9ca3af', color: '#6b7280', padding: '4px', borderRadius: '4px' }}
                      >
...
const App = () => {
  ...
  useEffect(() => {
    try {
      ...
      // Replaced anonymous sign-in with simple onAuthStateChanged listener
      const unsubscribe = onAuthStateChanged(auth, async (user) => {
        if (user) {
          setUserId(user.uid);
          ...
          setAuthReady(true);
          setLoadingData(true); // Re-trigger data load after successful login

        } else {
          // User is logged out. Show AuthView.
          setAuthReady(false);
          setUserId(null);
          setLoadingData(false);
          setUserRoleDefined(false); // Reset role status
          setCurrentView('Auth'); // Set view to the new login screen
        }
      });

      // Check if user is already authenticated
      if (auth.currentUser) {
        onAuthStateChanged(auth, () => { }, () => { }); // Trigger listener immediately if authenticated
      } else {
        // If no user is present on start, ensure loading is set to false to show login form
        setLoadingData(false);
      }
  ...
  // Initial loading state (only for Firebase init or data fetch)
  if (!authReady && !userId && loadingData) {
    return (
      <div style={{ minHeight: '100vh', backgroundColor: 'var(--color-gray-50)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <LoadingSpinner />
      </div>
  ...
  // Auth View (Show if not authenticated)
  if (!userId && currentView === 'Auth') {
    return (
      <div style={{ minHeight: '100vh', backgroundColor: 'var(--color-gray-50)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <AuthView setAuthReady={setAuthReady} setUserId={setUserId} />
      </div>
    );
  }

  // Role Selection Screen (Show if authenticated but role not defined)
  if (userId && !userRoleDefined) {
    return (
      <div style={{ minHeight: '100vh', backgroundColor: 'var(--color-gray-50)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <RoleSelectionView userId={userId} setIsShopkeeper={setIsShopkeeper} setUserRoleDefined={setUserRoleDefined} />
      </div>
  ...
  if (isShopkeeper) {
    ...
    switch (currentView) {
      ...
      case 'Profile':
        ...
        break;
      default:
        shopContent = <ShopkeeperDashboard userId={userId} storeId={linkedStoreId} setStoreId={setLinkedStoreId} setLinkedStoreId={setIsShopkeeper} stores={stores} shopOrders={shopOrders} newOrderCount={newOrderCount} products={products} />;
    }
